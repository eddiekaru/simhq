...
app.get('/api/bootstrap', auth, async (req, res, next) => {
  try {
    const [sims, agents, teams, usage, logs, users, damageReports] = await Promise.all([
      ...
      pool.query(`SELECT a.id,a.full_name AS name,a.employee_id AS "employeeId",a.phone,a.status,t.name AS team FROM agents a LEFT JOIN LATERAL (SELECT * FROM agent_team_history WHERE agent_id=a.id AND ended_at IS NULL LIMIT 1) h ON true LEFT JOIN teams t ON t.id=h.team_id WHERE $1 <> 'team_leader' OR t.leader_user_id=$2 ORDER BY a.full_name`, [req.user.role, req.user.id]),
