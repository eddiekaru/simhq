...
app.get('/api/bootstrap', auth, async (req, res, next) => {
  try {
    const [sims, agents, teams, usage, logs, users, damageReports] = await Promise.all([
      pool.query(`SELECT s.id,s.sim_number AS number,s.msisdn,s.iccid,s.provider AS network,s.sim_type AS type,s.current_status AS status,s.current_balance AS balance,s.notes,s.added_at AS "createdAt",a.assigned_at AS "assignedAt",ag.full_name AS agent,ag.employee_id AS "agentId",t.name AS team FROM sim_cards s LEFT JOIN sim_assignments a ON a.sim_id=s.id AND a.removed_at IS NULL LEFT JOIN agents ag ON ag.id=a.agent_id LEFT JOIN teams t ON t.id=a.team_id ORDER BY s.added_at DESC`),
      pool.query(`SELECT a.id,a.full_name AS name,a.employee_id AS "employeeId",a.phone,a.status,t.name AS team,COALESCE(history_leader.full_name,current_leader.full_name) AS "teamLeader" FROM agents a LEFT JOIN agent_team_history h ON h.agent_id=a.id AND h.ended_at IS NULL LEFT JOIN teams t ON t.id=h.team_id LEFT JOIN users history_leader ON history_leader.id=h.team_leader_id LEFT JOIN users current_leader ON current_leader.id=t.leader_user_id WHERE $1 <> 'team_leader' OR t.leader_user_id=$2 ORDER BY a.full_name`, [req.user.role, req.user.id]),
    ...
    const histories = await pool.query(`
      SELECT h.sim_id,h.occurred_at AS at,CONCAT('Status changed ',COALESCE(h.previous_status::text,'new'),' → ',h.new_status::text) AS action,h.notes AS note,u.full_name AS by
